...
describe('VDOM interop', () => {
  ...
  test('hydrate a vdom outlet fallback that is a fragment of its own behind the fragment of a forwarding outlet', async () => {
    const data = reactive({ show: false })
    const { container } = await testWithVaporApp(
      `${setup}<template>
        <components.Wrapper><b v-if="data.show">b</b></components.Wrapper>
      ...
      {
        Child: {
          code: `${setup}<template><slot><p v-for="i in 2">fallback</p></slot></template>`,
          vapor: false,
        },
        Mid: {
          code: `${setup}<template><components.Child><slot/></components.Child></template>`,
    ...
    // the range under the cursor reads as the slot's own: the fallback is
    // recreated, and nothing of the server's is left behind
    expect(`Hydration node mismatch`).toHaveBeenWarned()
    expect(`Hydration children mismatch`).toHaveBeenWarned()
    expect(visible(container)).toBe(`<p>fallback</p><p>fallback</p>`)
