...
describe('block real DOM insert/remove ops', () => {
  ...
  test('component array', async ({ bench }) => {
    await bench.compare(
      bench('ForBlock', () => {
        for (let i = 0; i < DOM_BATCH; i++) {
  ...
  test('slot-like fragment', async ({ bench }) => {
    await bench.compare(
      bench('ForBlock', () => {
        for (let i = 0; i < DOM_BATCH; i++) {
