...
describe('reactivity/reactive/Array', () => {
  ...
  // #3816
  test('add integer prop out of valid array length on Array should not trigger length dependency', () => {
    const array = new Array(3)
    const observed = reactive(array)
    const fn = jest.fn()
    effect(() => {
  ...
  // #3816
  test('using equal value to set integer prop out of valid array length on Array should trigger effect 1 times', () => {
    // integer key between 0 to 4294967295 will be treated as array index.
    // If a integer key is out of valid array length, it will be treated as a object key not a array index.
