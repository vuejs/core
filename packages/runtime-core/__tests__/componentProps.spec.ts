...
describe('component props', () => {
  ...
  it('props reserved key', () => {
    ...
    expect(
      ` Extraneous non-props attributes ($key,$ref,$slots) were passed to component but could not be automatically inhertied because component renders fragment or text root nodes`
    ).toHaveBeenWarned()
    expect(instanceProxy.key).toBeUndefined()
    expect(instanceProxy.ref).toBeUndefined()
    expect(instanceProxy.onVnodeBeforeMount).toBeUndefined()
  })
  ...
  it('no declared props move to attrs', () => {
    ...
    expect(instanceProxy.$attrs.obj2).toBe('obj2')
    expect(
      `Extraneous non-props attributes (obj2) were passed to component but could not be automatically inhertied because component renders fragment or text root nodes.`
    ).toHaveBeenWarned()
  })
